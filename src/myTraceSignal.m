function out = myTraceSignal(opts)
...
% input arguments
    arguments
        opts.direction (1,:) {mustBeMember(opts.direction,["forward", "back", "f", "b", "src", "dst"])} = 'forward'
        opts.block (1,1) = '';
        opts.porttype (1,:) {mustBeMember(opts.porttype,["inport","outport","i","o"])} = 'inport'
        opts.portNum = ':'
        opts.verbose (1,1) logical = true
        opts.enblHilite (1,1) logical = false
    end

    direction = opts.direction;
    block = myOp.slx.general.parseBlock(opts.block);
    porttype = opts.porttype;
    sigidx = opts.portNum;
    verbose = opts.verbose;
    enblHilite = opts.enblHilite;

    hBlock = block{1}.Handle;
    ...
    if strcmpi('f',direction) || strcmpi('dst',direction)
        % forward-trace
        traceProp = 'TraceDestinationInputPorts';
        traceDirection = 'forward';
    elseif strcmpi('b',direction) || strcmpi('src',direction)
        % back-trace
    ...
    beforeOpenSubSys = find_system(bdroot(hBlock),'LookUnderMasks','all',...
    ...
    set_param(bdroot(hBlock),'HiliteAncestors','none');
    ...
    if enblHilite
        ...
        afterOpenSubSys = find_system(bdroot(hBlock),'LookUnderMasks','all',...
            'FollowLinks','on','BlockType','SubSystem','Open','on');
        wasOpenedByFcn = setdiff(afterOpenSubSys,[beforeOpenSubSys;bdroot(hBlock)]);
